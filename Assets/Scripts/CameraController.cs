...
public class CameraController : MonoBehaviour
{
    ...
    [SerializeField] private float moveSpeedMax = 50;
    [SerializeField] private float moveSpeedMin = 5;
    private float _moveSpeed;
    [SerializeField] private float moveSmooth = 10;

    [SerializeField] private float zoomSpeedTouchMax = 10f;
    [SerializeField] private float zoomSpeedTouchMin = 2f;
    private float _zoomSpeedTouch;
    [SerializeField] private float zoomSpeedMouseMax = 30;
    [SerializeField] private float zoomSpeedMouseMin = 15;
    private float _zoomSpeedMouse;
    [SerializeField] private float zoomSmooth = 10;
    ...
    public void Initialize(Vector3 center, float horizontalMax, float horizontalMin, float verticalMax,
        float verticalMin, float angle, float zoom, float zoomMin, float zoomMax)
    {
        _center = center;
        _horizontalMax = horizontalMax;
        _horizontalMin = horizontalMin;
        _verticalMax = verticalMax;
    ...
    private void Update()
    {
        ...
        if (Input.touchSupported == false || true)
        {
            float mouseScroll = _inputs.Main.MouseScroll.ReadValue<float>();
            if (mouseScroll > 0)
            {
                _zoom -= _zoomSpeedMouse * Time.deltaTime;
            }
        ...
        if (_zooming)
        {
            ...
            _zoom = _zoomBaseValue - (deltaDistance * _zoomSpeedTouch);
        ...
        {
            _camera.orthographicSize = Mathf.Lerp(_camera.orthographicSize, _zoom, zoomSmooth * Time.deltaTime);
        }

        if (_camera.transform.position != _target.position)
        {
            _camera.transform.position = Vector3.Lerp(_camera.transform.position, _target.position,
    ...
    private void AdjustBoundValues()
    {
        float minFactor = _zoomMin / _zoomMax;
        float t = (_zoom - _zoomMin) / (_zoomMax - _zoomMin);
        
        _left = _right = Mathf.Lerp(_horizontalMin, _horizontalMax, t);
        _up = _down = Mathf.Lerp(_verticalMin, _verticalMax, t);

        _moveSpeed = Mathf.Lerp(moveSpeedMin, moveSpeedMax, t);
        _zoomSpeedMouse = Mathf.Lerp(zoomSpeedMouseMin, zoomSpeedMouseMax, t);
        _zoomSpeedTouch = Mathf.Lerp(zoomSpeedTouchMin, zoomSpeedTouchMax, t);
    }
    ...
    private Vector3 CameraScreenPositionToWorldPosition(Vector2 position)
    {
        ...
        Vector3 ancher = _camera.transform.position - (_camera.transform.right.normalized * w / 2f) -
